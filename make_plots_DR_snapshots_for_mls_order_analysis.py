    ...
    for p_order in range(1, max_order + 1):
        ...
        for k in tqdm(range(n), desc="Iterating"):
            ...
            for i, snapshot in enumerate(mls.storage):
                ...
                a1_rel_norms[i] = norm(a1 - mls_compute_from_fit(snapshot.data[:k + 1],
                                                                 mls.a1_list[:k+1])) / norm(a1)
                a2 = snapshot["a2"]
                a2_norms[i] = norm(snapshot.data[k] * mls.a2_list[k]) / norm(a2)
                a2_norms2[i] = norm(snapshot.data[k] * mls.a2_list[k])
                a2_rel_norms[i] = norm(a2 - mls_compute_from_fit(snapshot.data[:k + 1],
                                                                 mls.a2_list[:k + 1])) / norm(a2)
                f1 = snapshot["f1_dir"]
                f1_norms[i] = norm(snapshot.data[k] * mls.f1_dir_list[k]) / norm(f1)
                f1_norms2[i] = norm(snapshot.data[k] * mls.f1_dir_list[k])
                f1_rel_norms[i] = norm(f1 - mls_compute_from_fit(snapshot.data[:k + 1],
                                                                 mls.f1_dir_list[:k + 1])) / norm(f1)
                f2 = snapshot["f2_dir"]
                f2_norms[i] = norm(snapshot.data[k] * mls.f2_dir_list[k]) / norm(f2)
                f2_norms2[i] = norm(snapshot.data[k] * mls.f2_dir_list[k])
                f2_rel_norms[i] = norm(f2 - mls_compute_from_fit(snapshot.data[:k + 1],
                                                                 mls.f2_dir_list[:k + 1])) / norm(f2)
        ...
        plt.figure("a - 1")
        plt.title(f"sum a - norm - order = {p_order}")
        plt.semilogy(a1_mean_rel_norms, "x--", label="$\\|\\|A_1(\\mu)-\\sum_{i}g_qA_{1q}\\|\\|/\\|\\|A_1(\\mu)\\|\\|$")
        plt.semilogy(a2_mean_rel_norms, "x--", label="$\\|\\|A_2(\\mu)-\\sum_{i}g_qA_{2q}\\|\\|/\\|\\|A_2(\\mu)\\|\\|$")
        plt.xlabel("$q$")
        plt.grid()
        ...
        plt.figure("f - 1")
        plt.title(f"sum f - norm - order = {p_order}")
        plt.semilogy(f1_mean_rel_norms, "x--", label="$\\|\\|f_1(\\mu)-\\sum_{i}g_qf_{1q}\\|\\|/\\|\\|f_1(\\mu)\\|\\|$")
        plt.semilogy(f2_mean_rel_norms, "x--", label="$\\|\\|f_2(\\mu)-\\sum_{i}g_qf_{2q}\\|\\|/\\|\\|f_2(\\mu)\\|\\|$")
        plt.xlabel("$q$")
        plt.grid()
        plt.legend(loc=9, bbox_to_anchor=(0.5, -.13), ncol=2)
